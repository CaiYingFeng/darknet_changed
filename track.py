...
import os
f=open('/media/autolab/disk_3T/caiyingfeng/6DOF/F1/front.txt')#时间戳位姿
f_dof=list(f)
f.close

f=open('/media/autolab/disk_3T/caiyingfeng/darknet/front.txt')#图片名
im_name=list(f)
im_name.sort()
f.close

i_path=Path('/media/autolab/disk_3T/caiyingfeng/map/model',f'images.txt')#要保存的model的images.txt
conn = sqlite3.connect("/media/autolab/disk_3T/caiyingfeng/map/front.db")#db文件地址
cursor = conn.cursor()
